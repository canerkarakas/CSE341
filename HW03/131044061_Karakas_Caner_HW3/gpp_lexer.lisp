...
(defun toList1 (line index result)
   (if (or (equal (nth index line) #\Space) (equal (nth index line) nil))
      (return-from toList1 result)
   )
   (setf result (append result (list (nth index line))))
   (toList1 line (+ index 1) result)
)

(defun toListForIntegers1 (line index result)
   (if (or (equal (nth index line) #\Space) (equal (nth index line) nil))
      (return-from toListForIntegers1 result)
   )
   (if (equal (member (nth index line) integers) nil)
      (return-from toListForIntegers1 nil)
   )
   (setf result (append result (list (nth index line))))
   (toListForIntegers1 line (+ index 1) result)
)

;if has syntax error because of integers, find space index for jump
(defun goSpace1 (line index result)
   (if (or (equal (nth index line) #\Space) (equal (nth index line) nil))
      (return-from goSpace1 result)
   )
   (setf result (+ result 1))
   (goSpace1 line (+ index 1) result)
)

(defun isKeyword1 (word)
   (let ((control))
      (dotimes (n (length keywords))
         ...
         (if (equal (length word) (length (nth n keywords)))
            (progn
               ...
               (if (equal control T)
                  (return-from isKeyword1 control)
               )
            )
         )
         (setf control nil)
   ...
   (let* ((word '()) (integerVal '()) (control nil) (control2 nil) (control3 nil))
      (dotimes (n (length line))
         ...
         (if (not (equal (member (nth n line) integers) nil))
            (progn
               (setf integerVal (toListForIntegers1 line n integerVal))
               (if (equal integerVal nil)
                  (progn
                     (setf n (+ n (goSpace1 line n 0)))
                     (setf control T)
            ...
            (progn
               (if (not (equal (member (nth n line) alphabet) nil))
                  (progn
                     ...
                     (if (isKeyword1 word)
                        (setq tokens (append tokens (list (nth (indexOfKeyword1 word) keywordsTokens))))
...
(defun gppinterpreter2 (filename)
